...
namespace JumpGame.Model // 네임스페이스를 JumpGame.Model로 변경
{
    public class Obstruction
    {
        ...
        // ===== 불꽃 라인 관리용 변수 =====
        private List<Projectile> fireLines; // 여러 불꽃 라인을 담을 리스트
        ...
        public Obstruction()
        {
            ...
            // ===== 불꽃 라인 목록 초기화 =====
            fireLines = new List<Projectile>();

            // 수정된 초기화: (startX, startY, fireSpeed, image)
            // startX는 FIRE_START_X (753)여야 하고, startY는 원하는 Y 좌표여야 합니다.
            fireLines.Add(new Projectile(FIRE_START_X, 417, FIRE_SPEED, defaultFireBallImage));
            fireLines.Add(new Projectile(FIRE_START_X, 300, FIRE_SPEED, defaultFireBallImage));
            fireLines.Add(new Projectile(FIRE_START_X, 200, FIRE_SPEED, defaultFireBallImage));
        ...
        {
            // --- 불꽃 라인 업데이트 로직 ---
            foreach (Projectile line in fireLines)
            {
